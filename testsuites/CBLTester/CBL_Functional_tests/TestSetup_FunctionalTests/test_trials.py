...
def scope_collection_test_fixture(params_from_base_test_setup):
    ...
    channels = ["ABC"]
    data = {"bucket": bucket, "scopes": {"abhay": {"collections": {"collection1": {}}}}, "num_index_replicas": 0, "username": "travel-sample", "password": "password"}
    sg_db = "db"
    ...
    if pre_test_user_exists is False:
        sg_client.create_user(sg_admin_url, sg_db, sg_username, sg_password, channels=channels, auth=auth)
...
def test_sync_scopeA_colA_to_scopeA_colA(scope_collection_test_fixture, teardown_doc_fixture):
    ...
    log_info(doc_obj.getId(col_obj.getDocument(created_collection, doc_id)))
    channels_sg = ["ABC"]
    db.create_bulk_docs(2, "cbl", db=cbl_db, channels=channels_sg, collection=created_collection)

    # setup replicator
    replicator = Replication(base_url)
    session, replicator_authenticator, repl = replicator.create_session_configure_replicate(base_url, sg_admin_url, sg_db, sg_username, sg_password, channels_sg, sg_client, cbl_db, sg_blip_url, continuous=True, replication_type="push", auth=auth, collection=created_collection)

    log_info("Replication stopped")
    sg_docs = sg_client.get_all_docs(url=sg_url, db=sg_db, auth=session)
    log_info(sg_docs)
    db.deleteDB(cbl_db)
    # teardown_doc_fixture(sg_client, sg_admin_url, sg_db, doc_id, session)
    # exercise + verification
    # try:
    #     sg_client.get_doc(sg_url, sg_db, doc_id, auth=session, scope=scope, collection=collection)
    ...
    # # exercise + verification
    # try:
    #     sg_client.get_doc(sg_url, sg_db, doc_id, auth=session, collection=collection)
    # except Exception as e:
    ...
    #     sg_client.get_doc(sg_url, sg_db, doc_id, auth=session, scope="_default", collection=collection)
    # e.match("Not Found")
    # log_info("completed uptill now")
    assert True

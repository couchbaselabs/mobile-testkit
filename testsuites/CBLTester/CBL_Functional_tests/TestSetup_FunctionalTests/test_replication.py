...
@pytest.mark.parametrize("num_of_docs, continuous, x509_cert_auth", [
    pytest.param(10, True, False, marks=pytest.mark.sanity),
    (100, True, False),
    (1000, True, True)
])
def test_replication_configuration_valid_values(params_from_base_test_setup, num_of_docs, continuous, x509_cert_auth):
    ...
    # db.create_bulk_docs(num_of_docs, "cbl-2.7.0-Ghanta", db=cbl_db, channels=channels_sg)
    #
    ...
    # completed = replicator.getCompleted(repl)
    # replicator.stop(repl)
    print("Test is DONE CBL docs", cbl_db_docs, cbl_doc_count)
    # assert total == completed, "total is not equal to completed"

...
def test_scopes_and_collections_import_filters(scopes_collections_tests_fixture, params_from_base_test_setup):
    ...
    sg_client.add_docs(url=admin_client_1.admin_url, db=sg1["db"], number=3, id_prefix="should_be_in_sg2", channels=["A"], auth=auth)
    sg_client.add_docs(url=admin_client_1.admin_url, db=sg1["db"], number=3, id_prefix="should_not_be_in_sg2", channels=["A", "B"], auth=auth)
    ...
    replicator2_id = sg2["sg_obj"].start_replication2(
        local_db=sg2["db"] + "." + scope + "." + collection,
        remote_url=sg1["sg_obj"].url,
        remote_db=sg1["db"] + "." + scope + "." + collection,
    ...
    user_2_docs = sg_client.get_all_docs(url=admin_client_2.admin_url, db=sg2["db"], auth=auth, include_docs=True)

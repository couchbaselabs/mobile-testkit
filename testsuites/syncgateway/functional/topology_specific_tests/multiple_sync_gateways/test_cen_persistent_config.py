...
def test_named_and_default_group(params_from_base_test_setup, group_type):
    ...
    sg_obj = SyncGateway()
    non_persistent_sgconf_name = "sync_gateway_default"
    # sg_conf_name2 = "xattrs/no_import"
...
def test_roll_out_config_new_node(params_from_base_test_setup, setup_sgws):
    ...
    temp_sg_config = replace_string_on_sgw_config(temp_sg_config, str, replaced_group)
    copy_to_temp_cpc_config(temp_sg_config, mode)
    # cbs_cluster.reset(sg_config_path=temp_sg_config, bucket_list=bucket_list, use_config=True, sgdb_creation=False)
    sg1.stop()
    sg2.stop()
    sg3.stop()
    ...
    print("dbconfig json is after the bucket replace is ", dbconfig)

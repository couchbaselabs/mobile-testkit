...
def test_upgrade_delete_attachments(params_from_base_test_setup, sgw_version_reset, doc_count, marked):
    ...
    for i in range(6, 8):
        doc_id = "att_com_" + str(i)
        uploaded_doc = sg_client.get_raw_doc(sg_url, remote_db, doc_id=doc_id, auth=session)
    ...
    print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^config=" + str(admin_client.get_db_config(remote_db)))
    # 4. delete the documents.
    for i in range(6, 8):
        doc_id = "att_com_" + str(i)
        latest_rev = sg_client.get_latest_rev(sg_url, remote_db, doc_id, auth=session)
    ...
    # 7 Execute the compaction process is collecting the deleted docs
    assert sg_client.compact_attachments(sg_admin_url, remote_db, "status", auth=auth)["status"] == "completed"
    sg_client.compact_attachments(sg_admin_url, remote_db, "start", auth=auth)
    sg_client.compact_attachments(sg_admin_url, remote_db, "stop", auth=auth)
    # We need to wait for few minutes to start the process again
    # time.sleep(40)

    count = 1
    while True and count < 10:
        resp = sg_client.compact_attachments(sg_admin_url, remote_db, "start", auth=auth)
        try:
            resp["reason"]
        ...
        time.sleep(1)
        count += 1
    status = sg_client.compact_attachments(sg_admin_url, remote_db, "status", auth=auth)["status"]

    if status == "stopping" or status == "running":
        sg_client.compact_attachments(sg_admin_url, remote_db, "progress", auth=auth)
        if doc_count == 1000:
            time.sleep(120)
        else:
            time.sleep(40)
    log_info(sg_client.compact_attachments(sg_admin_url, remote_db, "status", auth=auth))
    assert sg_client.compact_attachments(sg_admin_url, remote_db, "status", auth=auth)["status"] == "completed"
    assert sg_client.compact_attachments(sg_admin_url, remote_db, "status", auth=auth)["marked_attachments"] >= marked, \
        "compaction count not matching"
    assert sg_client.compact_attachments(sg_admin_url, remote_db, "status", auth=auth)["purged_attachments"] == 45, \
        "purged attachments"
    assert sg_client.compact_attachments(sg_admin_url, remote_db, "status", auth=auth)["last_error"] == "", \
        "Error found while running the compaction process"

    assert sg_client.compact_attachments(sg_admin_url, remote_db, "status", auth=auth)["start_time"] != "", "Start time is empty"
    assert sg_client.compact_attachments(sg_admin_url, remote_db, "status", auth=auth)["compact_id"] != "", "Compact Id is empty"
...
def test_upgrade_purge_expire_attachments(params_from_base_test_setup, sgw_version_reset, delete_doc_type):
    ...
    if status == "running":
        sg_client.compact_attachments(sg_admin_url, remote_db, "progress", auth=auth)
        time.sleep(30)
    compaction_status = sg_client.compact_attachments(sg_admin_url, remote_db, "status", auth=auth)

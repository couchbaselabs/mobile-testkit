...
def still_tevst_verify_changes_compaction(params_from_base_test_setup, sg_conf_name):
    """
    @Summary
    1. Write a document
    2. Delete the document through SG's REST API
    3. Issue a changes request, verify that tombstone entry appears
    4. Wait for tombstone to expire, based on server metadata purge interval
    5. Issue a _compact call via SG's REST API
    6. Issue the same changes request, verify that tombstone entry does not appear
    """

...
def test_bucket_online_offline_resync_sanity(params_from_base_test_setup, sg_conf_name, num_users, num_docs,
    ...
    before_recieved_docs = in_parallel(user_objects, 'get_num_docs')
    # Add User
    log_info("Add docs")
    bulk = True
    ...
    org_docs_num = list(before_recieved_docs.values())
    ...
    if sync_gateway_version < "3.0.0":
        ...
        while resync_status['payload']['status'] != "completed" and retries < 50:
        ...
        if retries == 50:
            assert resync_status['payload']['status'] == "completed", "resync status never reported as 'completed'"

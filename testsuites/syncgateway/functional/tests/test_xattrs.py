...
def test_document_resurrection(params_from_base_test_setup, sg_conf_name, deletion_type):
    ...
    Tombstone / Resurrect through Sync Gateway
    Sync Gateway monitor expected revs for SDK Delete (wait for this) / Create doc (wait for this)
    SDK should see of not see the doc
    """
    cluster_conf = params_from_base_test_setup['cluster_config']
    cluster_topology = params_from_base_test_setup['cluster_topology']
    mode = params_from_base_test_setup['mode']

...
@pytest.mark.parametrize("sg_conf_name, num_docs, num_revisions", [
    ...
    ("sync_gateway_default_functional_tests_couchbase_protocol_using_port", 50, 100),
])
def test_longpoll_changes_parametrized(params_from_base_test_setup, sg_conf_name, num_docs, num_revisions):
    ...
    if ("sync_gateway_default_functional_tests_no_port" in sg_conf_name or "sync_gateway_default_functional_tests_couchbase_protocol_using_port" in sg_conf_name) and get_sg_version(cluster_conf) < "1.5.0":
...
@pytest.mark.parametrize("sg_conf_name, num_docs, num_revisions", [
    ...
    ("sync_gateway_default_functional_tests_couchbase_protocol_using_port", 10, 10)
])
...
def test_longpoll_awaken_channels(params_from_base_test_setup, sg_conf_name):
    ...
    # Get starting sequence of docs, use the last seq to progress past any _user docs.
    adam_changes = client.get_changes(url=sg_url, db=sg_db, since=0, feed="longpoll", auth=adam_auth, timeout=3)
    traun_changes = client.get_changes(url=sg_url, db=sg_db, since=0, feed="longpoll", auth=traun_auth, timeout=3)
    andy_changes = client.get_changes(url=sg_url, db=sg_db, since=0, feed="longpoll", auth=andy_auth, timeout=3)

    # Inconsistency filed - https://github.com/couchbase/sync_gateway/issues/2258
    if mode == "cc":
        # Make sure _user docs shows up in the changes feed
        assert len(adam_changes["results"]) == 1 and adam_changes["results"][0]["id"] == "_user/adam"
        assert len(traun_changes["results"]) == 1 and traun_changes["results"][0]["id"] == "_user/traun"

...
def discover_authenticate_endpoint(sg_url, sg_db, provider):
    ...
    logging.info("Invoking _oidc_challenge against: {}".format(oidc_challenge_url))
...
def test_openidconnect_basic_test(sg_url, sg_db, is_admin_port):
    ...
    - Go to the openid connect "mock" endpoint and tell it to return an oidc id
    - Make a request using the oidc id and make sure returns 200 OK
    - Make another request using the session cookie that is present in the oidc response

    Parameters:

    sg_url looks like http://<host>:<port>

    """

    if not is_admin_port:
        # make a request against the db and expect a 401 response
    ...
    logging.info("refresh_token: {}".format(refresh_token))
    ...
    cookie_to_send = extract_cookie(set_cookie_response)

    # make a request using the  cookie against the db and expect a 200 response
    db_url = "{}/{}".format(sg_url, sg_db)
    resp = requests.get(db_url, cookies=cookie_to_send)
    log_r(resp)
    assert resp.status_code == 200, "Expected 200 response"

    # make a request using the session_id that's sent in the body
    resp = requests.get(db_url, cookies={"SyncGatewaySession": response_json["session_id"]})
    assert resp.status_code == 200, "Expected 200 response"

    # now try to use the refresh token to get a few new id_tokens
    id_tokens = [ id_token ]
    for i in xrange(3):
        ...
        response_json = response.json()
        id_token_refresh = response_json["id_token"]
...
def test_openidconnect_no_session(sg_url, sg_db):
    ...
    """
    Tests using non-default provider which is configured to not
    return a session
    """
    ...
    logging.info("Sending request to authenticate_endpoint: {}".format(authenticate_url))

    # Make the request to _oidc_testing
    response = requests.post(authenticate_url, files=formdata)
    log_r(response)
    logging.info("reponse headers: {}".format(response.headers))
...
def test_openidconnect_expired_token(sg_url, sg_db):
    ...
    """
    Create a token but have it expire in a few seconds, then try to use it and make sure it
    doesn't work.
    """
    ...
    print("making request with id token: {}".format(id_token))
    # make a request using the ID token against the db and expect a 200 response
    headers = {"Authorization": "Bearer {}".format(id_token)}
    db_url = "{}/{}".format(sg_url, sg_db)
    resp = requests.get(db_url, headers=headers)
    logging.info("rseponse code: {}".format(resp.status_code))
    print("print response code: {}".format(resp.status_code))
    log_r(resp)
...
def test_openidconnect_garbage_token(sg_url, sg_db):
    ...
    """
    Create a garbage and make sure it doesn't work
    """
...
def test_openidconnect_invalid_scope(sg_url, sg_db):
    ...
    """
    Use OpenID Test provider with an invalid scope defined and expect
    it to fail to give an ID token
    """

    try:
        discover_authenticate_endpoint(sg_url, sg_db, "testinvalidscope")
    except HTTPError:
        logging.info("got expected HTTPError trying to get the authenticate endpoint")

...
def test_system_test(params_from_base_test_setup):
    ...
    sg_db_list = params_from_base_test_setup['sg_db_list']
    sg_url_list = params_from_base_test_setup['sg_url_list']
    sg_admin_url_list = params_from_base_test_setup['sg_admin_url_list']
    sync_gateway_version = params_from_base_test_setup['sync_gateway_version']
    up_time = int(params_from_base_test_setup['up_time'])
    ...
    log_info('> up_time                   = {}'.format(up_time))
    ...
    sg_replicate_enabled = False

    cluster_obj = cluster.Cluster(config=cluster_config)
    if len(cluster_obj.sync_gateways) > 1:
        sg_replicate_enabled = True

    if sg_replicate_enabled:
        sg1 = cluster_obj.sync_gateways[0]
        sg2 = cluster_obj.sync_gateways[1]

        remote_url = "{}/{}".format(sg2.admin.admin_url, sg_db_list[1])
        sg_repl_1 = sg1.start_replication2(replication_id="replicator_sequoia",
                                           local_db=sg_db_list[0],
    ...
    lb_url = sg_url_list[0]
    sg_admin_url = sg_admin_url_list[0]
    sg_db = sg_db_list[0]
    ...
    if sg_replicate_enabled:
        sg1.stop_replication2_by_id("replicator_sequoia", sg_db_list[0])
...
def create_user_names(num_users):
    ...
    num_per_type = int(num_users / len(USER_TYPES))
    user_names = []
...
def update_docs(sg_url, sg_db, users, update_runtime_sec, batch_size, docs_per_user_per_update, update_delay):
    ...
    while True:
        ...
        current_user_index = int((current_user_index + batch_size) % num_users_per_type)

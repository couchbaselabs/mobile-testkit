...
def generate_cluster_configs_from_vagrant(private_network, public_network, public_network_ethernet, ipv6):
    ...
    for stat in status:
        ...
        if ipv6:
            output = subprocess.check_output('vagrant ssh {} -c "ip -6 addr" | grep inet6'.format(name), shell=True)
            ip_addresses = re.findall("inet6 (.*?)\/64.*", output)
            public_ip = "[{}]".format(ip_addresses[1])
        else:
...
if __name__ == "__main__":
    ...
    parser.add_option("--ipv6",
                      ...
                      help="Generate pool.json/configs with IPv6 addresses")
